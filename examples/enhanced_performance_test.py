...
def benchmark_repeated_access():
    ...
    standard_dataset = StreamingDataset(
        input_dir="s3://pl-flash-data/optimized_tiny_imagenet", storage_options=storage_options
    )
    ...
    for epoch in range(num_epochs):
        ...
        if hasattr(inmemory_dataset.item_loader, "get_memory_stats"):
            ...
            print(f"    💾 Memory: {stats['memory_utilization']:.1%} ({stats['num_chunks_cached']} chunks)")
...
def benchmark_memory_efficiency():
    ...
    dataset = StreamingDataset(
        input_dir="s3://pl-flash-data/optimized_tiny_imagenet", item_loader=loader, storage_options=storage_options
    )
    ...
    for i in range(min(50, len(dataset))):  # Test more samples to better show memory usage
        try:
            ...
            if i % 10 == 0:
                ...
                print(
                    f"  Sample {i:2d}: {stats['memory_utilization']:.1%} memory "
...
def create_performance_report() -> Dict[str, Any]:
    ...
    try:
        ...
        report["tests"]["repeated_access"] = {
            ...
            "improvement_pattern": [standard_times[i] / inmemory_times[i] for i in range(len(inmemory_times))],
        }
